...
// tipo y valores para nuestro caso: bmp en modo bgr de 24 bits con los pixeles por filas bottom to
// top y columnas left to right
...
void
PixMap32RGBA::load_headers(std::ifstream& stream,
                           unsigned int& width,
                           unsigned int& height) const
{
	...
	{
		throw std::ios_base::failure(
		  "PixMapRGBA::load_bmpBGR() ERROR: Invalid depth color (bits per pixel)");
	}
	...
	{
		throw std::ios_base::failure(
		  "PixMapRGBA::load_bmpBGR() ERROR: Invalid number of colors in the palette");
	}
}

void
PixMap32RGBA::save_headers(std::ofstream& stream) const
{
	bitmap_file_header bfh;
	bitmap_information_header bih;
...
PixMap32RGBA::PixMap32RGBA(PixMap32RGBA&& pixmap)
  : data_{pixmap.data_}
...
void
PixMap32RGBA::reserve(GLsizei width, GLsizei height)
{
	if (width > width_ || height > height_) {
		try {
			...
			throw std::runtime_error(
			  "PixMap32RGBA::reserve() ERROR: Could not allocate memory (bad_alloc)");
		}
	}
}

void
PixMap32RGBA::load_bmp24BGR(const std::string& file_name)
{
...
void
PixMap32RGBA::row_bgr_rgba(char* rowAux, unsigned int r)
{
...
void
PixMap32RGBA::row_rgba_bgr(char* rowAux, unsigned int r) const
{
	unsigned int j = 0, f = r * width_ + width_;
...
void
PixMap32RGBA::save_bmp24BGR(const std::string& file_name) const // throw(std::ios_base::failure)
{
...
void
PixMap32RGBA::set_alpha(GLubyte alpha)
{
	// if(data_ != nullptr)
	for (GLsizei i = 0; i < width_ * height_; i++)
		data_[i].a = alpha;
...
void
PixMap32RGBA::set_colorkey_alpha(glm::u8vec3 colorKey, GLubyte alpha)
{
	// if (data_ != nullptr)
	for (GLsizei i = 0; i < width_ * height_; i++) {
