...
void
Texture::bind(GLuint mixMode) // GL_REPLACE, GL_MODULATE, GL_ADD
{
	glBindTexture(GL_TEXTURE_2D, mId);
...
void
Texture::setWrap(GLuint wp) // GL_REPEAT, GL_CLAMP
{
	glBindTexture(GL_TEXTURE_2D, mId);
...
// APARTADO 32
void
Texture::load(const std::string& BMP_Name, glm::u8vec3 color, GLubyte alpha) {
	if (mId == 0)
		init();
	...
	glm::u8vec4* r = pixMap.data();

	mWidth = pixMap.width();
	mHeight = pixMap.height();

	PixMap32RGBA modifiedPixMap();
...
void 
Texture::save(const std::string filename) const{
	PixMap32RGBA pixMap;
	glReadBuffer(GL_FRONT);
	//glBindTexture(GL_TEXTURE_2D, mId);
	glReadPixels(0, 0, mWidth, mHeight, GL_RGBA, GL_UNSIGNED_BYTE, pixMap.data());
	//glGetTexImage(GL_TEXTURE_2D, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixMap.data());
	pixMap.save_bmp24BGR(filename);
	//glBindTexture(GL_TEXTURE_2D, 0);
